...
- 当前闸门条目 **66**，2026-09-19 全绿（`test-results/regression-2026-09-19.md`）。
- `test/` 目录共 **73** 个脚本文件；闸门外 **7** 个 = 设计如此 **3**（性能测量 / 演示物料 / 需真实 Key）
  + 真欠债 **1**（`verify-composer`）+ 单跑入口 **3**（`run-regression.mjs` 是闸门本身、`verify-current-build`
  与 `verify-security` 由 `npm run verify:build` / `verify:security` 单独调）。
  上一版这里写「共 72 个、闸门外 5 个」，漏数了闸门自身与两个单跑入口，一并更正。
...
## 真欠债只剩 1 条（2026-09-17 取证，2026-09-18/19 陆续结案）
...
5. 只剩 1 条（`verify-composer`，14/21）。它的「生成按钮完成(done)」断言与 genmeta-retry 同根因（出图需 `page.route` 打桩），其「比例下拉写回 / 跟随节点定位」两条要先确认新版避让逻辑下的正确预期，不能简单放宽。`verify-auto-connect` 已于 2026-09-19 结案（18/18 入闸门），定级翻案的经过见「真欠债」表与第 4 处缺陷。
...
node test/run-regression.mjs          # 全量闸门（66）
