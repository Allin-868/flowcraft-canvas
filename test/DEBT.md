...
与第 5 处同类（点外部 / Esc 两条关闭路径全缺），但不能照抄模板面板的「任何外部点击都关」：场景卡上的「加入选中(N)」按钮依赖「面板开着 → 画布点选节点 → 回面板点加入」这条主流程，点节点即关会把它打断。修复契约：点面板外关闭，例外两类——`#btnScenes`（保 toggle）与 `.node` 本体（保选节点）；全局 Esc 链补场景面板一档。闸门 `verify-scene-panel-close.mjs` 7 条断言，修复前反向验证恰好「画布空白点击关 / Esc 关」2 条红。

注：曾按「太靠近画布顶部、尽量在画面中心」反馈把面板改成右侧垂直居中浮窗（提交 1180ddc），用户随后明确「场景面板还是维持原来的样子」——该反馈实际指向当时的原生 prompt 对话框（第 7 处缺陷），已回退面板形态、保留对话框居中方案（提交待记）。教训：**「弹窗位置」类反馈先确认说的是哪个窗口**，别顺手把相邻容器一起重排。
...
### 第 7 处缺陷（2026-09-19 用户截图报告）：场景分组「新建 / 加入场景」用浏览器原生 prompt

`prompt('场景名称')` 与 `prompt('把这 N 个节点加入哪个场景？输入序号')` 是浏览器原生对话框：Chrome 把它们钉在窗口左上角（用户原话「太靠近画布顶部区域了，尽量在画面中心」），白框与深色主题割裂，且原生框无法由页面移位或样式化——唯一解是换成应用内浮层。新增通用 `showAppInputDialog`（输入型：Enter/确定提交、Esc/点遮罩/取消关闭、自动聚焦全选；Esc 捕获阶段 stopPropagation，不连带关掉后面的场景面板）与 `showAppChoiceDialog`（选择型：场景渲染成可点卡片，消灭「输入序号」反直觉交互），替换场景流两处调用；场景面板「点外部关」处理器同步放行 `.app-dialog-overlay`（对话框挂在 body 下，否则点对话框按钮的 mousedown 会先把面板关掉）。应用内其余原生 prompt（工作流/工作台重命名、提供方编辑等）登记为待迁移此助手的候选，本轮不动。闸门 `verify-scene-dialogs.mjs` 10 条断言（含 `page.on('dialog')` 兜底计数零原生弹窗），修复前反向验证 8 红 2 绿，红的恰是缺失能力。

### 第 8 处缺陷（2026-09-19 由第 70 条闸门用例暴露）：「加入选中 / 加入场景」把 undefined 写进场景成员
