...
const NODE_TYPES = {
  ...
  audio:     { label: '音频',  desc: '配音 / 配乐（一个节点二合一·模式切换）', color: 'var(--node-voiceover)', colorRaw: '#1D9E75', inputs: [{type:'text',label:'脚本'}], outputs: [{type:'audio',label:'音频'}], defaultParams: { audioMode: 'voiceover', text: '', lang: 'zh-CN', speed: '1.0', voice: '晓晓', duration: '', name: '', volume: 100, mood: '轻快', loopSeconds: 8 } },
...
const NODE_ICONS = {
  ...
  audio:     '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 8h1.2M5.5 4.5v7M8.5 2.5v11M11.5 5.5v5M14 8h-1"/></svg>',
...
const CURRENT_AUTOSAVE_VERSION = 6;
...
function migrateAutosave(parsed, fromVersion) {
  ...
  // v5 → v6：配音/配乐整合为单一「音频」节点（模式切换）。旧类型保留为兼容别名，
  // 但存档中的既有实例统一迁移为 audio，避免画布/节点库出现两套音频节点。
  if (v < 6) {
    (parsed.nodes || []).forEach(nd => {
      if (!nd) return;
      if (nd.type === 'voiceover') { nd.type = 'audio'; nd.params = nd.params || {}; nd.params.audioMode = 'voiceover'; }
      else if (nd.type === 'bgm') { nd.type = 'audio'; nd.params = nd.params || {}; nd.params.audioMode = 'bgm'; }
    });
  }
...
const TEMPLATES = [
  ...
  },
  {
    ...
    nodes: [
      ...
      { type: 'audio', x: -480, y: 120, title: 'AI 配音', params: { audioMode: 'voiceover' } },
      { type: 'subtitle', x: -180, y: 120, title: '生成字幕', params: {} },
      { type: 'audio', x: 120, y: 120, title: '添加配乐', params: { audioMode: 'bgm' } },
...
function buildNodeComposerBody(node) {
  ...
  if (!isMediaInput) {
    ...
    // 同参重试入口。节点体内不再渲染生成信息条（纯图片视图），而 retrySameParams 原先
    // 只挂在信息条里（legacy.js:5296/5314）→ 对 aiImage/imageEdit 等于无入口。
    // 与右键菜单同一套做法，仅在已有成功快照时出现。
    if (node.genMeta) {
      const retryBtn = document.createElement('button');
      retryBtn.type = 'button';
      retryBtn.className = 'nc-run nc-retry';
      retryBtn.textContent = '↻ 同参重试';
      retryBtn.title = '用上次成功生成的完全相同参数（提示词/模型/比例/分辨率/张数）重新运行本节点';
      retryBtn.onmousedown = (e) => e.stopPropagation();
      retryBtn.onclick = (e) => { e.stopPropagation(); retrySameParams(node); };
...
function restoreFromRecycle(index) {
  ...
  node.previewOnly = !!nd.previewOnly;
  node.previewSourceId = typeof nd.previewSourceId === 'string' ? nd.previewSourceId : '';
  if (['videoInput', 'image', 'bgm', 'audio'].includes(node.type)) {
...
function buildSidebar() {
  ...
  ['script', 'footage', 'audio', 'subtitle', 'compose', 'publish'].forEach(type => {
    list.appendChild(createLibraryItem(type));
  });
}
...
function buildNodeBody(el, node) {
  ...
  // —— 视频生成节点 body ——
  const videoTypes = ['script', 'footage', 'voiceover', 'subtitle', 'bgm', 'audio', 'compose', 'publish'];
  if (videoTypes.includes(node.type)) {
    ...
    else if (node.type === 'voiceover') buildVoiceoverBody(node, vbody);
    ...
    else if (node.type === 'bgm') buildBgmBody(node, vbody);

    else if (node.type === 'audio') {
      const p = node.params = node.params || {};
      p.audioMode = (p.audioMode === 'bgm') ? 'bgm' : 'voiceover';
      const tabs = document.createElement('div'); tabs.className = 'fc-audio-tabs';
      const mkTab = (mode, label) => { const b = document.createElement('button'); b.type = 'button'; b.className = 'fc-audio-tab' + (p.audioMode === mode ? ' active' : ''); b.textContent = label; b.onmousedown = (e) => e.stopPropagation(); b.onclick = (e) => { e.stopPropagation(); if (p.audioMode === mode) return; p.audioMode = mode; scheduleAutosave(); if (node.el) buildNodeBody(node.el, node); }; return b; };
      tabs.appendChild(mkTab('voiceover', '\uD83C\uDFA4 \u914D\u97F3'));
      tabs.appendChild(mkTab('bgm', '\uD83C\uDFB5 \u914D\u4E50'));
      vbody.appendChild(tabs);
      if (p.audioMode === 'bgm') buildBgmBody(node, vbody); else buildVoiceoverBody(node, vbody);
    }
...
function renderConnectionNodeMenu() {
  ...
  const groups = [
    ...
    { key: 'video',   label: '视频',    class: 'hud-group-video',   types: ['videoBreak', 'script', 'footage', 'audio', 'subtitle', 'compose', 'publish'] },
  ];
...
function collectUpstreamScriptText(node, depth) {
  ...
  for (const s of srcs) {
    ...
    if ((s.type === 'voiceover' || (s.type === 'audio' && s.params && s.params.audioMode !== 'bgm')) && String(s.params && s.params.text || '').trim()) return String(s.params.text).trim();
...
//================ B 批音频节点补齐 · 配音 / 生成音乐（2026-09-19） ================
function fcArrayBufferToB64(buf) {
...
// 配音音色（UI 中文名）→ OpenAI /audio/speech voice id；未知回落 nova
const VOICEOVER_TTS_VOICE = {
  '晓晓': 'nova', '晓伊': 'nova', '云希': 'echo', '云健': 'onyx', '云西': 'echo', '晓辰': 'alloy', '晓北': 'shimmer',
  'alloy': 'alloy', 'echo': 'echo', 'fable': 'fable', 'onyx': 'onyx', 'nova': 'nova', 'shimmer': 'shimmer',
};
function mapVoiceoverVoice(v) {
  const s = String(v || '').trim();
  if (VOICEOVER_TTS_VOICE[s]) return VOICEOVER_TTS_VOICE[s];
  const low = s.toLowerCase();
  return ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'].indexOf(low) >= 0 ? low : 'nova';
}
function probeAudioDuration(dataUrl) {
  return new Promise((res) => {
    const a = new Audio();
    const t = setTimeout(() => res(''), 6000);
    a.preload = 'metadata';
    a.onloadedmetadata = () => { clearTimeout(t); res(formatAudioDuration(isFinite(a.duration) ? a.duration : 0)); };
    a.onerror = () => { clearTimeout(t); res(''); };
    a.src = dataUrl;
  });
}
function estimateSpeechDuration(text, speed) {
  const secs = Math.ceil(Array.from(String(text)).length / (4.2 * Math.max(0.5, speed || 1)));
  return formatAudioDuration(Math.max(1, secs));
}
// 配音：有 Key 且未走代理时直连 OpenAI 兼容 /audio/speech 真实合成 mp3（二进制→dataURL）；
// 无 Key / 代理不支持二进制 / 接口报错时回落浏览器 speechSynthesis 试听（不产出文件，语义 local，下游拿不到声，如实反映）。
async function runVoiceoverNode(node) {
  const text = String((node.params && node.params.text) || '').trim() || collectUpstreamScriptText(node);
  if (!text) return null;
  node.params.text = text;
  const speed = Math.max(0.5, Math.min(2, Number(node.params && node.params.speed) || 1));
  const voice = mapVoiceoverVoice(node.params && node.params.voice);
  const key = localStorage.getItem(OPENAI_KEY_STORAGE);
  if (key && !_fcProxy()) {
    ...
    try {
      const resp = await fetch(base + '/audio/speech', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key },
        body: JSON.stringify({ model: 'tts-1', input: text.slice(0, 4000), voice: voice, speed: speed, response_format: 'mp3' }),
      });
      if (resp.ok) {
        const buf = await resp.arrayBuffer();
        if (buf && buf.byteLength > 128) {
          const b64 = fcArrayBufferToB64(buf);
          if (b64) {
            const dataUrl = 'data:audio/mpeg;base64,' + b64;
            const dur = await probeAudioDuration(dataUrl);
            return { mode: 'real', src: dataUrl, duration: dur || estimateSpeechDuration(text, speed), mime: 'audio/mpeg', tts: true };
          }
        }
      }
    } catch (e) { /* 网络/CORS → 回落试听 */ }
  }
...
function buildVoiceoverBody(node, vbody) {
      const p = node.params = node.params || {};
...
function buildBgmBody(node, vbody) {
      const p = node.params = node.params || {};
...
const BGM_MOODS = {
  ...
  '轻快': { bpm: 104, wave: 'triangle', root: 261.6, scale: [0, 2, 4, 6, 7, 9, 11], pad: false },
  '紧张': { bpm: 92, wave: 'sawtooth', root: 174.6, scale: [0, 1, 5, 6, 8], pad: false },
  '欢快': { bpm: 120, wave: 'square', root: 293.7, scale: [0, 2, 4, 7], pad: false },
  '沉静': { bpm: 60, wave: 'sine', root: 196.0, scale: [0, 3, 5, 7, 10], pad: true },
};
function bgmMood(name) { return BGM_MOODS[String(name || '').trim()] || BGM_MOODS['轻快']; }
function mulberry32(a) { return function () { a |= 0; a = a + 0x6D2B79F5 | 0; let t = Math.imul(a ^ a >>> 15, 1 | a); t = t + Math.imul(t ^ t >>> 7, 61 | t) ^ t; return ((t ^ t >>> 14) >>> 0) / 4294967296; }; }
function encodeWavDataUrl(audioBuf) {
  try {
    const ch = Math.min(2, audioBuf.numberOfChannels);
    const sr = audioBuf.sampleRate;
    const frames = audioBuf.length;
    const blockAlign = ch * 2, dataLen = frames * blockAlign;
    ...
    return 'data:audio/wav;base64,' + fcArrayBufferToB64(buf);
  } catch (e) { return null; }
}
// 生成音乐：用 OfflineAudioContext 按情绪在浏览器内离线渲染一段免版权循环，编码为 WAV dataURL（纯本地真实出音，零 Key/CORS/超时）。
async function runBgmGenerateNode(node) {
  const moodName = String((node.params && node.params.mood) || '').trim() || '轻快';
  const safeMood = BGM_MOODS[moodName] ? moodName : '轻快';
  const mood = bgmMood(safeMood);
  const secs = Math.max(4, Math.min(12, Number(node.params && node.params.loopSeconds) || 8));
  const OfflineCtx = window.OfflineAudioContext || window.webkitOfflineAudioContext;
  if (!OfflineCtx) return null;
  const sr = 22050;
  let ctx;
  try { ctx = new OfflineCtx(2, Math.floor(sr * secs), sr); } catch (e) { return null; }
  const beat = 60 / mood.bpm, note = beat / 2;
  const master = ctx.createGain(); master.gain.value = 0.9;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = (mood.wave === 'square' || mood.wave === 'sawtooth') ? 2200 : 5200;
  master.connect(lp); lp.connect(ctx.destination);
  const scale = mood.scale;
  const degFreq = (idx) => { const oct = Math.floor(idx / scale.length); const step = scale[((idx % scale.length) + scale.length) % scale.length]; return mood.root * Math.pow(2, oct + step / 12); };
  function voiceAt(freq, start, dur, gainVal, type) {
    try {
      const o = ctx.createOscillator(); o.type = type || mood.wave; o.frequency.value = freq;
  ...
  for (let b = 0, t = 0; t < secs - beat * 0.5; b++, t += beat) voiceAt(degFreq(b - 7) , t, beat * 0.9, 0.28, 'sine');
  let seedv = 0; for (let i = 0; i < safeMood.length; i++) seedv += safeMood.charCodeAt(i);
  const rnd = mulberry32(seedv + secs * 7919);
  let dg = 4;
  for (let n = 0, t = note; t < secs - note * 0.5; n++, t += note) {
    dg += Math.floor(rnd() * 5) - 2; dg = Math.max(0, Math.min(scale.length * 2, dg));
    if (rnd() > 0.22) voiceAt(degFreq(dg + 7), t, note * 0.85, 0.16, mood.wave);
  }
  if (mood.pad) { voiceAt(degFreq(0), 0, secs, 0.10, 'sine'); voiceAt(degFreq(2), 0, secs, 0.08, 'sine'); }
  let rendered;
  try { rendered = await ctx.startRendering(); } catch (e) { return null; }
  const wav = encodeWavDataUrl(rendered);
  if (!wav) return null;
...
function computeNodeOutput(node) {
  ...
  switch (node.type) {
    ...
    case 'voiceover':
      ...
      break;
    case 'audio': {
      const amode = (node.params && node.params.audioMode) === 'bgm' ? 'bgm' : 'voiceover';
      if (amode === 'bgm') out[0] = { type: 'audio', value: { src: node.uploadedAudio || '', name: node.params.name || '', duration: node.params.duration || '', volume: Number(node.params.volume ?? 100), mime: node.params.mime || '' } };
      else out[0] = { type: 'audio', value: { name: '配音', duration: node.params.duration } };
      break;
    }
    case 'subtitle':
  ...
  return new Promise(resolve => {
    ...
    if (window.FlowCraft && window.FlowCraft.nodes && window.FlowCraft.nodes.beforeRun) {
      ...
      try {
        ...
        // 配音：有 Key 直连 /audio/speech 真实合成 mp3；无 Key 回落浏览器试听（B 批补齐，替代纯占位）
        if (node.type === 'voiceover') {
          const vo = await runVoiceoverNode(node);
          if (vo) {
            if (vo.src) {
              node.uploadedAudio = vo.src; // 复用与配乐一致的音频管线
              node.params.duration = vo.duration;
              node.params.mime = vo.mime;
              node.params.name = 'AI 配音';
              delete node._voPreview;
              node.outputsData = [{ type: 'audio', value: { src: vo.src, name: 'AI 配音', duration: vo.duration, volume: 100, mime: vo.mime } }];
            } else {
              node.params.duration = vo.duration;
              node._voPreview = true; // 仅试听标记（节点体据此渲染试听按钮）
              node.outputsData = [{ type: 'audio', value: { name: '配音（仅试听·未导出）', duration: vo.duration, volume: 100, preview: true } }];
            }
            setNodeResultMode(node, vo.mode);
            node.status = 'done';
            captureGenMeta(node, Date.now() - _t0);
            logRun(node, true, Date.now() - _t0);
            if (node.el) buildNodeBody(node.el, node);
        ...
        // 生成音乐：OfflineAudioContext 真实合成循环（B 批补齐，替代仅手动上传）；已上传音频则如实保留为本地真实声源
        if (node.type === 'bgm') {
          ...
          const gen = await runBgmGenerateNode(node);
          if (gen) {
            node.uploadedAudio = gen.src;
            node.params.name = gen.name;
            node.params.duration = gen.duration;
            node.params.mime = gen.mime;
            node.outputsData = computeNodeOutput(node);
            setNodeResultMode(node, 'local');
            node.status = 'done';
            captureGenMeta(node, Date.now() - _t0);
        ...
        // 音频节点（配音/配乐二合一·按 audioMode 分派；整合批）
        if (node.type === 'audio') {
          const amode = (node.params && node.params.audioMode) === 'bgm' ? 'bgm' : 'voiceover';
          if (amode === 'bgm') {
            if (node.uploadedAudio) {
              node.outputsData = computeNodeOutput(node); setNodeResultMode(node, 'local'); node.status = 'done';
              logRun(node, true, Date.now() - _t0); if (node.el) buildNodeBody(node.el, node);
              updateNodeStatus(node); markEdgesDirty(); scheduleAutosave(); resolve(); return;
            }
            const agen = await runBgmGenerateNode(node);
            if (agen) {
              node.uploadedAudio = agen.src; node.params.name = agen.name; node.params.duration = agen.duration; node.params.mime = agen.mime;
              node.outputsData = computeNodeOutput(node); setNodeResultMode(node, 'local'); node.status = 'done';
              captureGenMeta(node, Date.now() - _t0); logRun(node, true, Date.now() - _t0); if (node.el) buildNodeBody(node.el, node);
              refreshConnectedSavePreviews(node); updateNodeStatus(node); markEdgesDirty(); scheduleAutosave(); resolve(); return;
            }
          } else {
            const avo = await runVoiceoverNode(node);
            if (avo) {
              if (avo.src) { node.uploadedAudio = avo.src; node.params.duration = avo.duration; node.params.mime = avo.mime; node.params.name = 'AI 配音'; delete node._voPreview; node.outputsData = [{ type: 'audio', value: { src: avo.src, name: 'AI 配音', duration: avo.duration, volume: 100, mime: avo.mime } }]; }
              else { node.params.duration = avo.duration; node._voPreview = true; node.outputsData = [{ type: 'audio', value: { name: '配音（仅试听·未导出）', duration: avo.duration, volume: 100, preview: true } }]; }
              setNodeResultMode(node, avo.mode); node.status = 'done';
              captureGenMeta(node, Date.now() - _t0); logRun(node, true, Date.now() - _t0); if (node.el) buildNodeBody(node.el, node);
              refreshConnectedSavePreviews(node); updateNodeStatus(node); markEdgesDirty(); scheduleAutosave(); resolve(); return;
            }
          }
...
const MENU_ICONS = {
  ...
  retry:     '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M13 8a5 5 0 1 1-1.46-3.54M13 2.5V5h-2.5"/></svg>',
...
const CTX_NODE_SECTIONS = [
  ...
  { label: '视频成片', types: ['videoBreak', 'footage', 'audio', 'subtitle', 'compose', 'publish'] },
...
function showContextMenu(x, y, node) {
  ...
  if (node) {
    ...
    addMenuSeparator();
    if (node.genMeta) {
      addMenuItem('同参重试', MENU_ICONS.retry, '', () => retrySameParams(node));
    }
